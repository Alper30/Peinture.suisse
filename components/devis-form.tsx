 ...
 */
function draftToMessage(draft: DevisDraft, t: (key: string) => string): string {
  const header = (
    [
      [t("name"), draft.name],
      [t("phone"), draft.phone],
...
export function DevisForm() {
  ...
  /**
   * Başarı ekranı ile form arasındaki geçiş BİLEREK animasyonsuz değiştiriliyor.
   *
   * Önceden burada `AnimatePresence mode="wait"` vardı: yeni çocuğu, eskisinin
   * exit animasyonu bitene kadar DOM'a koymaz. React 19'da `useActionState`
   * güncellemesi bir transition içinde geldiği için o "exit bitti" sinyali hiç
   * ulaşmıyordu — state `success` olmasına rağmen form ekranda kalıyor, React
   * alanları temizliyor ve müşteri talebinin gidip gitmediğini anlamıyordu.
   * (Canlıda doğrulandı: action `{"status":"success"}` döndü, ekran değişmedi.)
   *
   * Bu, sitedeki TEK dönüşüm onayı. Bir çıkış animasyonu uğruna üçüncü parti
   * bir kütüphanenin zamanlama sinyaline bağlamıyoruz: giriş animasyonu kalıyor,
   * çıkış anında oluyor. Form da artık düz `<form>` — `action` prop'unu React
   * doğrudan görüyor, arada sarmalayıcı yok.
   */
  return (
